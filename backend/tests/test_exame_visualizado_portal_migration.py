...
MIGRATION_PATH = (
    BACKEND_DIR / "migrations" / "versions" / "20260815_67_exame_visualizado_portal.py"
)
SPEC = importlib.util.spec_from_file_location("migration_20260815_67", MIGRATION_PATH)
...
class ExameVisualizadoPortalMigrationTest(unittest.TestCase):
    def _build_engine(self):
        tmpdir = tempfile.TemporaryDirectory()
        db_path = Path(tmpdir.name) / "exame-visualizado-portal-migration.db"
    ...
    def test_upgrade_adiciona_a_coluna(self) -> None:
        ...
        try:
            ...
            self.assertIn("visualizado_portal_em", columns)
        finally:
            engine.dispose()
            tmpdir.cleanup()
