...
import { resetPartnerPassword } from "@/lib/portal-api";
import { PORTAL_PASSWORD_MIN_LENGTH, validatePortalPasswordConfirmation } from "@/lib/portal-password";

type PortalPartnerResetPasswordWorkspaceProps = {
  resetToken: string;
};

export default function PortalPartnerResetPasswordWorkspace({
  resetToken,
}: PortalPartnerResetPasswordWorkspaceProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await resetPartnerPassword({
  ...
  if (!resetToken) {
    return (
      <div className="fc-portal-auth-card rounded-lg border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-950">
        O link de redefinição está incompleto. Solicite um novo e-mail de acesso pelo portal do parceiro.
      </div>
    );
  }
      ...
      {done ? (
        <div>
          ...
          <p className="mt-3 text-sm leading-6 text-slate-600">
            Sua senha foi redefinida. No próximo login, o portal pode pedir uma confirmação adicional para garantir que o acesso voltou ao profissional correto.
          </p>
          <Link
          ...
          >
            Voltar para o login do parceiro
          </Link>
        </div>
      ) : (
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div>
            ...
            <h2 className="mt-3 text-2xl font-bold text-slate-950">
              Escolha uma nova senha para o portal do parceiro
            </h2>
          </div>
