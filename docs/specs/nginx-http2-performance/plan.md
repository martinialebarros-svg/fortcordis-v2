...
7. Preparar e disparar o inventario manual somente-leitura: socket `:443`,
   topologia efetiva de `nginx -T` limitada a arquivo, `listen`, `server_name`
   e `http2`, mais probes ALPN dos dois hosts do app. O job compartilha a trava
   de deploy e nao pode ser executado fora de `stage`.
