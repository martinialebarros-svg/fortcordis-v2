...
| CA-004 / RF-005 | aceitacao | campo segue controlado por `setDataExame` com `onChange`; o preenchimento automatico usa `current || ...`, entao valor digitado nao e sobrescrito. Revisao de codigo + verificacao em stage | ok |
...
Executados em stage pelo responsavel em 2026-09-10, com resultado positivo.
O responsavel confirmou o funcionamento sem detalhar cenario a cenario, entao
os itens abaixo ficam registrados individualmente como nao reportados.

- Cenario 1 (ok, confirmado pelo responsavel): abrir a agenda, escolher um
  agendamento de eletrocardiograma de data passada e clicar no atalho de laudo.
  "Data de realizacao" traz a data do agendamento.
- Cenario 2 (nao reportado individualmente): abrir Laudos > "Upload de
  eletrocardiograma" pelo menu, sem agendamento. Esperado: data de hoje.
  Coberto pelo teste automatizado "cai para a data do dia quando o upload nao
  tem agendamento".
- Cenario 3 (nao reportado individualmente): abrir o upload com
  `?agendamento_id=` de um id inexistente. Esperado: data de hoje e o aviso
  "Nao foi possivel carregar o contexto do agendamento." Coberto pelo teste
  automatizado "cai para a data do dia quando o agendamento nao carrega".
- Cenario 4 (nao reportado individualmente): trocar a data manualmente e enviar
  o PDF. Esperado: o laudo grava a data digitada. O campo continua controlado
  por `onChange` e o preenchimento automatico usa `current || ...`, entao nao
  ha caminho de sobrescrita; nao bloqueia a aprovacao.
- Cenario 5 (nao reportado individualmente): abrir o ecocardiograma
  estruturado por um agendamento de data passada
  (`/laudos/novo?agendamento_id=<id>`). Esperado: "Data do exame" com a data do
  agendamento. Mudanca menor nesse fluxo (normalizacao com `calendarDateInput`
  e fallback para `inicio`); a prioridade da data do agendamento ja era o
