...
| CA-017 | aceitacao/seguranca | `frontend/lib/portal-password.ts` valida o minimo de 8 caracteres; `frontend/lib/portal-errors.ts` traduz erro estruturado sem reproduzir o campo `input`; telas de ativacao e redefinicao aplicam ambos os controles | ok |
...
- Frontend:
  - Regressao isolada do payload Pydantic da senha curta: a resposta permanece normalizada para `A senha deve ter pelo menos 8 caracteres.` sem reproduzir o valor presente em `input`.
