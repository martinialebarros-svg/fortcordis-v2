...
if ! jq -e '(.code_verification_status // "VERIFIED") == "VERIFIED"' <<<"${phone_response}" >/dev/null; then
  fail "Numero de stage ainda nao esta verificado na Meta"
fi
...
printf '[OK] Token resolve o numero de stage verificado na Cloud API\n'
...
unset phone_response waba_phones_response subscribed_apps_response WHATSAPP_ACCESS_TOKEN
